<?php
    ...
    if ($id) {
        $select = "SELECT * FROM pegawai WHERE id_pegawai = '$id'";
        $sql = $db->query($select);
    ...
    INNER JOIN jadwalKerja as d ON a.id_jadwal_kerja = d.id_jadwal_kerja) WHERE id_pegawai = '$id'";
    ...
    if (isset($_POST['save'])) {
        ...
        $sqlInsertData = "UPDATE pegawai SET 
        ...
        if ($executeInsert) {
            echo "
                <script>    
                    alert('Edit Data Pegawai Berhasil');
